...
#define maskmaxx 346
#define maskmaxy 20
void ProcessingExceptionVideo(std::string filepath);
void PreprocessingImage(const cv::Mat& frame, cv::Mat& dst);
void EraseLidar(cv::Mat& frame);
std::pair<int, int> ConnectComponent(const cv::Mat& frame, std::vector<std::vector<int>>& ex_lines);
void CreateCsvFile(const int frame_count, const std::vector<std::vector<int>> line);
int main()
{
	std::string filepath = "video/Sub_project.avi";
	cv::Mat calibratied_frame;
	ProcessingExceptionVideo(filepath);
}
void ProcessingExceptionVideo(std::string filepath) {
	...
	while (1) {
		...
		PreprocessingImage(roi, roi_th_gray);
		pos = ConnectComponent(roi_th_gray, ex_lines);
	...
	if (frame_count % 30 == 0) {
		tm.stop();
			std::cout << frame_count << " frame time : " << tm.getTimeMilli() << std::endl;
			tm.reset();
			tm.start();
			line.push_back({ frame_count, pos.first, pos.second });
	}
	frame_count++;

}
cv::destroyAllWindows();
CreateCsvFile(frame_count, line);
}
void CreateCsvFile(const int frame_count, const std::vector<std::vector<int>> line) {
